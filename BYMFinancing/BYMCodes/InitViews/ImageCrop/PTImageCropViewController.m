...
@interface PTImageCropViewController ()
{
    UIView* m_bgView;
...
@implementation PTImageCropViewController
...
- (void)initLayout
{
    m_bgImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, SCREENWIDTH, SCREENHEIGHT)];
    ...
    // todo 临时使用
    UIView* barView = [[UIView alloc] initWithFrame:CGRectMake(0, SCREENHEIGHT-64, SCREENWIDTH, 64)];
    [barView setBackgroundColor:[UIColor whiteColor]];
    barView.alpha = 0.1;
    [self.view addSubview:barView];
    
    UIButton* rltButton = [UIButton buttonWithType:UIButtonTypeCustom];
    rltButton.frame = CGRectMake(12, SCREENHEIGHT-64 + 10, 100, 44);
    [rltButton setTitle:@"取消" forState:UIControlStateNormal];
    [rltButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [rltButton addTarget:self action:@selector(returnBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:rltButton];
    
    UIButton* comBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    comBtn.frame = CGRectMake(SCREENWIDTH - 112, SCREENHEIGHT-64 +10, 100, 44);
